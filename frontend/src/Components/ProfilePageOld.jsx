...
const API_BASE = process.env.REACT_APP_API_URL || "https://localhost:443"; // Adjust as needed if hosted on a real server
...
const ProfilePage = () => {
  ...
  const [contentReady, setContentReady] = useState(false); // Controls the rest of the page
    ...
    const load = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/api/user/purchases`, {
          headers: {
            Authorization: `Bearer ${token}`, // Sends the user token for authentication, stops unauthorized access to a user's library.
          },
        });
