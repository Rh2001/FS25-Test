...
const StoreGamesGrid = memo(function StoreGamesGrid({ storeGames, onDelete }) {
  return (
...
const StoreGamesManagement = ({ authHeader, onMessage }) => {
  ...
  const handleAddGame = async (e) => {
    ...
    if (!store.trim()) newErrors.store = "Store is required.";
  ...
  return (
    <>
      ...
      >
        ...
        <form onSubmit={handleAddGame} className="grid gap-3 md:grid-cols-2">
          ...
          <div>
            ...
            />
            {errors.store && (
              <p className="mt-1 text-xs text-rose-400">{errors.store}</p>
            )}
          </div>
      ...
      <StoreGamesGrid storeGames={storeGames} onDelete={requestDelete} />
